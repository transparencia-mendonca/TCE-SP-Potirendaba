...
const DBN="tcesp-potirendaba-v6",STORE="dados",KEY="base";
...
function tceExpenseUrl(year){return `https://transparencia.tce.sp.gov.br/municipio/potirendaba/${year}/despesas`}
...
const MUNICIPIO_API="potirendaba";
...
(async()=>{
  ...
  try{
    ...
    if(!hasUsableData(db)){
      let old=null;
      try{
        let raw=localStorage.getItem("tcesp-potirendaba-data")||localStorage.getItem("tcesp-data");
    ...
    try{
      await save();
      localStorage.removeItem("tcesp-potirendaba-data");
    }catch(e){
      initError=e;
